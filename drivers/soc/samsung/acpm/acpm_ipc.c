...
#else
	if (acpm_debug->apm0_dump_size)
		memcpy(acpm_debug->apm0_dump_dram_base, acpm_debug->apm0_dump_base, acpm_debug->apm0_dump_size);

	if (acpm_debug->apm1_dump_size)
		memcpy(acpm_debug->apm1_dump_dram_base, acpm_debug->apm1_dump_base, acpm_debug->apm1_dump_size);
#endif
}
...
static int __acpm_ipc_request_channel(struct acpm_ipc_info *ipc,
...
int acpm_ipc_request_channel(struct device_node *np, ipc_callback handler,
		unsigned int *id, unsigned int *size)
{
...
int esca_ipc_request_channel(struct device_node *np, ipc_callback handler,
		unsigned int *id, unsigned int *size)
{
	if (esca_ipc)
...
#else
	prop = of_get_property(node, "apm0-dump-base", &len);
	if (prop)
		dump_base = be32_to_cpup(prop);

	prop = of_get_property(node, "apm0-dump-size", &len);
	if (prop)
		dump_size = be32_to_cpup(prop);

	if (dump_base && dump_size) {
		acpm_debug->apm0_dump_base = ioremap(dump_base, dump_size);
		acpm_debug->apm0_dump_size = dump_size;
	}

	acpm_debug->apm0_dump_dram_base = kzalloc(acpm_debug->apm0_dump_size, GFP_KERNEL);
	dbg_snapshot_printk("[ACPM] acpm framework SRAM dump to dram base: 0x%x\n",
			virt_to_phys(acpm_debug->apm0_dump_dram_base));

	dbg_snapshot_add_bl_item_info("apm0_dram", virt_to_phys(acpm_debug->apm0_dump_dram_base),
						acpm_debug->apm0_dump_size);

	prop = of_get_property(node, "apm0-dram-dump-base", &len);
	if (prop) {
		dram_dump_base = be32_to_cpup(prop);
		dbg_snapshot_add_bl_item_info("apm0_sram", dram_dump_base, acpm_debug->apm0_dump_size);
	}
	pr_info("[ACPM] acpm framework SRAM dump to dram base: 0x%llx\n",
			virt_to_phys(acpm_debug->apm0_dump_dram_base));

	prop = of_get_property(node, "apm1-dump-base", &len);
	if (prop)
		dump_base = be32_to_cpup(prop);

	prop = of_get_property(node, "apm1-dump-size", &len);
	if (prop)
		dump_size = be32_to_cpup(prop);

	if (dump_base && dump_size) {
		acpm_debug->apm1_dump_base = ioremap(dump_base, dump_size);
		acpm_debug->apm1_dump_size = dump_size;
	}

	acpm_debug->apm1_dump_dram_base = kzalloc(acpm_debug->apm1_dump_size, GFP_KERNEL);
	dbg_snapshot_printk("[ACPM] acpm framework SRAM dump to dram base: 0x%llx\n",
			virt_to_phys(acpm_debug->apm1_dump_dram_base));

	dbg_snapshot_add_bl_item_info("apm1_dram", virt_to_phys(acpm_debug->apm1_dump_dram_base),
						acpm_debug->apm1_dump_size);

	prop = of_get_property(node, "apm1-dram-dump-base", &len);
	if (prop) {
		dram_dump_base = be32_to_cpup(prop);
		dbg_snapshot_add_bl_item_info("apm1_sram", dram_dump_base, acpm_debug->apm1_dump_size);
	}
	pr_info("[ACPM] acpm framework SRAM dump to dram base: 0x%llx\n",
			virt_to_phys(acpm_debug->apm1_dump_dram_base));
...
int acpm_ipc_probe(struct platform_device *pdev)
{
	...
	if (len > 0) {
		...
		if (of_property_read_u32_array(node, "interrupt-ch", ch_buf, len)) {
			...
			dev_info(&pdev->dev, "interrupt channels buf %llx, len %d\n", (u64)ch_buf, len);
		}
	} else {
		ch_buf = NULL;
		dev_info(&pdev->dev, "interrupt channels empty\n");
