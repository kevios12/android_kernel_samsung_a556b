 ...
 * Dst depends on framerate and waits 3 frames time
 */
#define MFC_DEC_DEFER_WAIT_SRC	(MFC_DEC_DEFER_FRAME + 2)
#define MFC_DEC_DEFER_WAIT_DST	3
/* The max framerate deferred decoding supports */
#define MFC_DEC_DEFER_MAX_FPS	60000
/* Deferred decoding buffer interval (msec) */
#define MFCBUF_TIMER_INTERVAL	200
/* The initial skip frame for boost and deferred decoding */
...
#define CODEC_10BIT_DEC(ctx)	(IS_HEVC_DEC(ctx) || IS_VP9_DEC(ctx) ||	\
				IS_BPG_DEC(ctx) || IS_AV1_DEC(ctx))
#define CODEC_10BIT_ENC(ctx)	(IS_HEVC_ENC(ctx) || IS_VP9_ENC(ctx) ||	IS_BPG_ENC(ctx))
